type RenderedElement = HTMLElement | DocumentFragment;
type Status = 'completed' | 'pending' | 'deleted';
interface ListOption {className: string, text: string, icon: string, clickEvent?: string};
interface CreateElement {name: string, attrs: Object, event?: [string, Function, ...any], elementsToAppend?: (HTMLElement|string)[]};

class ListItem {
    id: number;
    liElement: HTMLLIElement|HTMLElement;
    pElement: HTMLParagraphElement|HTMLElement;
    optionList: ListOption[];
    
    static todoList: ListItem[] = [];
    static ulElement: HTMLUListElement = document.querySelector('#list')!;
    static modalElement: HTMLElement = document.querySelector('#show_area .render-area')!;
    static filterMode = document.querySelector<HTMLSelectElement>('#filter')!.value;
    static currentTheme: 'light'|'dark' = 'light';

    constructor(public text: string, public status: Status = 'pending') {
    ...
    createElement({name, attrs, event, elementsToAppend}: CreateElement): HTMLElement {
        const element = document.createElement(name);
        const attrKeys = Object.keys(attrs);

        Object.entries(attrs).forEach(([key, value]) => {
            if(key === 'textContent') return element.textContent = value;
            if(key === 'innerHTML') return element.innerHTML = value;
            if(key === 'className') return element.classList.add(...value.split(' '));
            element.setAttribute(key, value);
        })
        
        if(event) element.addEventListener(event[0], e => event[1](e, event.slice(2)));
        if(elementsToAppend) element.append(...elementsToAppend)
        return element;
    }
    ...
    render() {
        ...
        this.optionList.forEach(el => {
            ...
            (el.clickEvent === 'up' || el.clickEvent === 'down' || el.clickEvent === 'edit')) return;
    ...
    renderEditor() {
        ...
        const input = this.createElement({name:'input', attrs:{placeholder: 'Enter new todo list item', className: 'edit-input'}}) as HTMLInputElement;
        ...
        form.addEventListener('submit', e => {
            ...
            }else alert('Invalid Todo List Item!');
        });
    ...
    handleClickEvents(event: Event, mode: string | undefined) {
        if(ListItem.toggleModal(true, true)) ListItem.toggleModal(false);
    ...
    moveDown() {
        let pos = ListItem.todoList.indexOf(this);
        if(pos === ListItem.todoList.length-1) return;
    ...
    delete() {
        if(this.status === 'deleted') {
            ...
            if(yesToDelete) ListItem.todoList = ListItem.todoList.filter(todo => todo !== this); this.liElement.remove();
    ...
    static init() {
        document.querySelector('.add-item-form')?.addEventListener('submit', e => {
            e.preventDefault();
            let addItemInput = document.querySelector<HTMLInputElement>('#add_item_input');
        ...
        document.querySelector('.clear-all-item')?.addEventListener('click', e => ListItem.clearTodoList(true));
        document.querySelector('.empty-trash')?.addEventListener('click', e => ListItem.emptyTrash(true));
        document.querySelector('.change-theme')?.addEventListener('click', e => ListItem.SwitchTheme());
        
        ListItem.SwitchTheme();
        ListItem.update(false);
    }

    static switchThemeValues = (a: string, b: string, force=false) => {
        ...
        }else root.style.setProperty(a, b);
    };
    ...
    static fetchList() {
        let todoList: ({ text: string, status: Status })[];
        try {
            todoList = JSON.parse(localStorage.getItem('simpleTodoList') ?? '[]');
        }catch { todoList = []; }
    ...
    static renderAll() {
        ...
        const filteredTodoList: ListItem[] = ListItem.todoList.filter(todo => {
            if(ListItem.filterMode === 'all' || ListItem.filterMode === undefined) {
                return todo.status === 'completed' || todo.status === 'pending';
            }
            if(ListItem.filterMode === 'everything') return true;
    ...
    static update(store=true, fetch=true, render=true) {
        if(store) ListItem.storeList();
        if(fetch) ListItem.fetchList();
        if(render) ListItem.renderAll();
    }

    static toggleModal(show=true, check=false) {
        const modalElement = ListItem.modalElement.parentElement?.parentElement!;
        if(check) return !modalElement.classList.contains('hide')
    ...
    static clearTodoList(safetyCheck=false) { 
        if(safetyCheck) {
            ...
            if (yesorno) ListItem.todoList.forEach( todo => todo.liElement.remove()); ListItem.todoList = []; ListItem.storeList();
            alert('Refresh to see changes');
        }else {
            ListItem.todoList.forEach( todo => todo.liElement.remove()); ListItem.todoList = []; ListItem.storeList();
        }
        ListItem.renderListInfo();
     }
    ...
    static emptyTrash(safetyCheck=false) { 
        if(safetyCheck) {
            ...
            if (yesorno) ListItem.todoList.forEach(todo => todo.status === 'deleted' ? todo.liElement.remove() : ''); ListItem.todoList = ListItem.todoList.filter(todo => todo.status !== 'deleted'); ListItem.storeList();
        }else {
            ListItem.todoList.forEach( todo => todo.liElement.remove()); ListItem.todoList = []; ListItem.storeList();
        }
        ListItem.renderListInfo();
     ...
     static renderListInfo() {
        const infoArea = document.querySelector('.status')! as HTMLDivElement;
